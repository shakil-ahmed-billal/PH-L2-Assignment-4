...
export const getProviderOrderStats = async (providerId) => {
    try {
        const [totalOrders, placedOrders, preparingOrders, readyOrders, deliveredOrders, cancelledOrders, totalRevenue,] = await Promise.all([
            prisma.order.count({
                where: { providerId },
            }),
            prisma.order.count({
 ...
 */
export const getRecentOrders = async (providerId, days = 7) => {
    try {
 ...
 */
export const getMealsByProviderId = async (providerId) => {
    try {
 ...
 */
export const toggleMealAvailability = async (mealId, available) => {
    try {
        const meal = await prisma.meal.update({
            where: {
                id: mealId,
 ...
 */
export const getProviderMealStats = async (providerId) => {
    try {
        const [totalMeals, availableMeals, popularMeals, vegetarianMeals, spicyMeals, avgPrice, totalReviews, avgRating,] = await Promise.all([
            prisma.meal.count({
                where: { providerId },
            }),
            prisma.meal.count({
 ...
 */
export const searchProviderMeals = async (providerId, searchQuery) => {
    try {
        const meals = await prisma.meal.findMany({
            where: {
                providerId: providerId,
...
export const getProviderDashboard = async (providerId) => {
    try {
        const [
        // Order stats
        totalOrders, placedOrders, preparingOrders, readyOrders, deliveredOrders, cancelledOrders, 
        // Meal stats
        totalMeals, availableMeals, popularMeals, 
        // Revenue stats
 ...
 */
export const getRevenueAnalytics = async (providerId, startDate, endDate) => {
    try {
        const orders = await prisma.order.findMany({
            where: {
                providerId,
 ...
 */
export const getTopSellingMeals = async (providerId, limit = 10) => {
    try {

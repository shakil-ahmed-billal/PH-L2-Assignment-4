...
const MealDetailsPage = ({ params }: MealDetailsPageProps) => {
  ...
  const [meal, setMeal] = useState<Meal | null>(null); // Update to Meal or null
  const { getMealById } = useData();
  ...
  const relatedMeals = meal
    ? getMealsByProvider(meal.providerId)
...
useEffect(() => {
  const fetchMealDetails = async () => {
    await getMealDetails(); // Wait for meal details to be fetched
    setIsLoading(false); // Set loading state to false after fetching data
  };

  fetchMealDetails();

  // No need for setTimeout or cleanup timer as we are handling async logic
}, [id]);
  ...
  const savings = meal.originalPrice
        ...
        <div className="relative">
          ...
          <div className="absolute top-4 left-4 flex flex-wrap gap-2">
            ...
            {meal.isVegetarian && (
              <Badge
                variant="secondary"
                className="shadow-lg backdrop-blur-sm bg-secondary/90"
              >
            ...
            {meal.isSpicy && (
              <Badge
                variant="destructive"
                className="shadow-lg backdrop-blur-sm bg-destructive/90"
              >
          ...
          >
            <Heart
              className={`h-5 w-5 transition-transform ${isWishlisted ? "fill-current scale-110" : ""}`}
            />
          </Button>
        </div>
          ...
          <div className="flex items-center gap-6 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">
                ({meal.reviewCount}{" "}
                {meal.reviewCount === 1 ? "review" : "reviews"})
              </span>
            </div>
